...
const BookingPage = () => {
  ...
  const [availableHours, setAvailableHours] = useState([
    ...
    "15:00",
    "16:00",
  ]); // Replace with your available hours ,"06:00","09:00","10:00","15:00","16:00"
  ...
  const handelBooking = async () => {
    ...
    axios
      .post("http://" + ipAddress + ":8000/addBooking", bookData)
      .then((response) => {
        alert(
  ...
  function filter_data(hours_that_taken) {
    const hours = availableHours.filter(
      (element) => !hours_that_taken.includes(element)
    );
    const nestedArray = [];
    console.log(hours);
  ...
  async function getDataFromMongiDB(myDay) {
    ...
    const response = await axios.post(
      "http://" + ipAddress + ":8000/getBookingData",
      data
    );
    ...
    <View style={styles.container}>
      <Pressable
        onPress={() =>
          router.push({
            pathname: page,
            params: { username: username, email: email, page: page },
          })
        }
      >
