...
const selectionPage = () => {
  ...
  const handelDelete = (item) => {
    ...
    axios
      .delete("http://" + ipAddress + ":8000/deleteItem", { data })
      .then((response) => {
        setRefresh(refresh + 1);
      });
  };
        ...
        {presentButton === item.Name && ButtonSetting && adminStatus && (
          <View>
            <View style={{ padding: 5 }}>
              <Pressable
                style={{ borderRadius: 7, backgroundColor: "red", padding: 5 }}
            ...
            <View style={{ padding: 5 }}>
              <Pressable
                style={{ borderRadius: 7, backgroundColor: "gray", padding: 5 }}
                onPress={() => alert(admin)}
              >
          ...
          >
            ...
            {adminStatus && (
              <View style={{ flexDirection: "column" }}>
                <Pressable
                  onPress={() => setNewTypeEx(!NewTypeEx)}
                  style={{ alignSelf: "center" }}
                >
                  <AntDesign name="pluscircle" size={30} color="black" />
                </Pressable>
                <Text style={{ paddingTop: 10, paddingLeft: 5 }}>
                  Add new experiments EMC type
                </Text>
              </View>
            )}
            <Pressable onPress={() => setRefresh(refresh + 1)}>
